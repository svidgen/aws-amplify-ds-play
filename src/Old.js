...
function App() {
  ...
  async function addData() {
    ...
    console.log("putting", newOrderProduct);
    const orderProduct = await DataStore.save(newOrderProduct);
  ...
  async function getData() {
    ...
    try {
      const wrongestWay = await DataStore.query(OrderProducts, (op) => {
        // debugger;
        const orderIDField = op.orderID;
        return orderIDField("eq", someOrder.id);
      });
